...
public class Etc {
	...
	public static void main(String[] args) {
		 ...
		 *  - ^(XOR) : 피연산자의 값이 서로 다를 때만 1을 같을 떄는 0을 결과로 얻는다. 
		 *  - ~(비트전환): 피연산자를 0은 1로 1은 0으로 바꾼다. 
		 *  - <<(쉬프트) : 피연산자의 각 자리를 왼쪽으로 이동시킨다. 빈칸은 0으로 채운다. 
		 *  - >>(쉬프트) : 피연산자의 각 자리를 오른쪽으로 이동시킨다. 빈칸은 앞자리와 동일한 값으로 채운다.
		 * 
		 *  << 기타 연산자  >>
		 *  - .(참조연산자) : 특정범위 내에 속해있는 맴버를 지정할 때 사용한다. 
		 *  - (type) : 형변환 (casting)
		 *  - ?:(삼항연산자): 조건식 ? 조건식 참일 경우 수행할 문장 : 조건식이 거짓일 경우 수행할 문장 
		 *  - instanceof: 참조형타입 확인 
		 *  
		 */
		
		/*
		 * - 10진수 -> 2진수 : 10진수 값을 1이 될 떄까지 2로 나눈다. 
		 * ex) 15를 2진수로 
		 *     7   -1 
		 *     3   -1 
		 *     1   -1 
		...
		//^ : 00000101
		System.out.println(10 ^ 15);
		//~ : 11110101   (10만을 가지고 연산)
		System.out.println(~10); //앞자리는 부호 1:- 0:+
		//<< : 00010100   (10을 가지고 연산)
		System.out.println(10 << 1);
		//>> : 00000101   (10을 오른쪽으로 한칸 이동)
		System.out.println(10 >> 1);
		...
		//변수에 저장된 주민등록변호의 뒷자리의 첫번째 숫자가 1이나 3이면 남자를 
		//2나 4면 여자를 출력해주세요 
		//그외의 숫자는 확인불가를 출력 
		regNo = 6;
		gender = regNo == 1 ? "남자" : (regNo == 3 ? "남자" : (regNo == 2 ? "여자" : regNo == 4 ? "여자" : "확인불가" ));
		System.out.println("당신의 성별은 " + gender + " 입니다.");
